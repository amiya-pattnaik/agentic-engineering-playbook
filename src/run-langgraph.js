...
import { outputPathForScenarioLangGraph, runScenarioLangGraph } from "./langgraph-orchestrator.js";
...
function printHelp() {
  console.log("Usage: node src/run-langgraph.js [scenario.json] [--no-write] [--metrics data/metrics.json] [--signals config/signals.json] [--html] [--run-tests]");
  console.log("Examples:");
  console.log("  node src/run-langgraph.js scenarios/banking-app.json --metrics data/metrics.json --signals config/signals.json --run-tests --html");
  console.log("  node src/run-langgraph.js --list");
}
...
async function main() {
  ...
  if (signalsPath) {
    try {
      const extraSignals = loadSignals(signalsPath);
      scenario.inputs = scenario.inputs || {};
  ...
  if (runTests) {
    try {
      ...
      if (testResults.summary) {
        ...
        testSignals.push(
          `Playwright tests: ${summary.expected ?? "?"} expected, ${summary.skipped ?? 0} skipped, failures ${summary.failures ?? summary.unexpected ?? 0}`
        );
      }
  ...
  const reportPath = shouldWrite ? outputPathForScenarioLangGraph(scenario) : undefined;
  const htmlPath = shouldWrite && htmlOutput ? reportPath.replace(/\.md$/, ".html") : undefined;

  console.log(`Running LangGraph scenario: ${scenario.title || scenarioPath}`);
  ...
  const { report, modelKind, engine } = await runScenarioLangGraph(scenario, {
  ...
  console.log(`\n--- Agentic AI Works Report (${engine}, ${modelKind}) ---\n`);
  console.log(report);
